    ...
    try {
        ...
        for (const message of messages) {
            ...
            try {
                ...
                const recursiveParse = (parts: any[]) => {
                    for (const part of parts) {
                        ...
                        // Handle content
                        if (part.mimeType === 'text/plain' && part.body.data && !bodyTextForAI) {
                        ...
                        if (part.parts) recursiveParse(part.parts);
                    }
                };
                ...
                if (payload?.parts) recursiveParse(payload.parts);

                if (!originalBody || originalBody.trim().length === 0) {
                    originalBody = "The email content could not be rendered, but attachments may be available below.";
                    bodyTextForAI = "No message body found.";
                }
